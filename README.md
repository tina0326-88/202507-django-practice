...
> 目前開發階段部分端點尚未加上權限控管，正式上線前請依實際需求加上 `IsAuthenticated` / `IsAdminUser` 等權限類別。
...
```json
{
  ...
  "student_id": 2,
  "course_id": 1,
  "enrolled_at": "2026-08-29T10:05:00Z"
}
```
...
執行單元測試，涵蓋 model 限制（如重複報名）與 API 端點：
...
- [ ] API 權限控管（老師/管理員專屬操作加上驗證）
- [ ] 分頁（courses / students 列表資料量大時）
- [ ] 前端搜尋與篩選課程功能
- [ ] 部署設定（正式環境資料庫、靜態檔案處理）
